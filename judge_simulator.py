...
BOT_URL = "https://magicpin-vera-engine-ltji.onrender.com"
...
# Disable SSL verification for macOS environments with missing certificates
import ssl
try:
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        try:
            ...
            try:
                ...
                print(f"[ERROR] LLM Request failed: {e.code} - {error_body}")
            except:
                print(f"[ERROR] LLM Request failed: {e}")
            raise
        except Exception as e:
            print(f"[ERROR] LLM Request Exception: {e}")
            raise
...
class MockProvider(LLMProvider):
    def name(self) -> str:
        return "Mock (Heuristic Judge)"

    def complete(self, prompt: str, system: str = None) -> str:
        # Heuristic scoring based on instructions in the prompt
        # We look at the 'BOT REPLY' part of the prompt
        match = re.search(r"BOT REPLY:\s*\"(.*?)\"", prompt)
        ...
        if len(reply) > 320:
            score -= 4
            critique.append("Message exceeds 320 chars.")
        if "http" in reply.lower():
            score -= 5
            critique.append("Found forbidden URL.")
        if "..." in reply[-5:]:
            score -= 1
            critique.append("Message was truncated.")
        if len(reply) < 10:
            score -= 6
            critique.append("Message is too short/empty.")
            
        reason = " ".join(critique) if critique else "Good adherence to constraints."
        
        return json.dumps({
            "score": max(1, score),
            "critique": f"[MOCK JUDGE]: {reason}"
        })
...
class JudgeSimulator:
    ...
    def run(self, scenario: str) -> bool:
        ...
        print_info("Wiping remote state...")
        self.client._request("GET", "/v1/teardown")
