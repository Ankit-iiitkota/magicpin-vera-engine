...
tries the primary first; the moment the primary raises, it logs once,
flips to "degraded", and both that call and every call after it go
straight to the fallback for the rest of the process's lifetime.
...
class ResilientContextStore(BaseContextStore):
    ...
    def __init__(self, primary: BaseContextStore, fallback: BaseContextStore) -> None:
